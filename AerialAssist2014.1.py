...
#Resolution of camera should be set to 800x600
#Do so in the camera settings 
...
def get_disp_to_wall(GlobalWidth,width):
        ...
        dist_to_wall = -0.0000000046233 * dist_to_wall*dist_to_wall +  0.00050503*dist_to_wall + 1.8753
        return dist_to_wall*1000
...
def get_blue_hue(img):
        ...
        #lower = [100, 200, 200]
        #Blue HSV
        #Upper- TopRange, Lower-LowRange
        #(Hue, Saturation, Value)
        #upper = np.array([255, 255, 180],np.uint8)
        #lower = np.array([60, 254, 40],np.uint8)
        #Green Values
        upper = np.array([90, 255, 150],np.uint8)
        lower = np.array([50, 255, 40], np.uint8)
...
def getDistFromCenter():
    ...
    if (grabFromCamera == True):
        ...
        img = cv2.imread('C:\PyGoalFinder\image.jpg')
           
    #Todo: Width/Height Backwards
    ...
    for i,cnt in enumerate(contours):
        ...
        #At 6 ft away, area is aprox 2350
        MinArea = 1000 
        MaxArea = 8400
